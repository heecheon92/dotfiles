...
static inline mach_port_t mach_get_bs_port() {
  ...
  kern_return_t result = bootstrap_look_up(bs_port,
...
static inline bool mach_receive_message(mach_port_t port, struct mach_buffer* buffer, bool timeout) {
...
static inline bool mach_send_message(mach_port_t port, char* message, uint32_t len) {
  ...
  mach_msg_return_t send_result = mach_msg(&msg.header,
  ...
  if (send_result == MACH_MSG_SUCCESS) {
    ...
    if (mach_receive_message(response_port, &buffer, true))
      mach_msg_destroy(&buffer.message.header);
  }

  mach_port_destroy(task, response_port);
  return send_result == MACH_MSG_SUCCESS;
}
...
static inline bool mach_server_begin(struct mach_server* mach_server, mach_handler handler, char* bootstrap_name) {
  ...
  while (mach_server->is_running) {
    if (!mach_receive_message(mach_server->port, &buffer, false)) continue;
...
static inline void sketchybar(char* message) {
  ...
  if (!g_mach_port) return;

  if (!mach_send_message(g_mach_port, formatted_message, caret + 1)) {
